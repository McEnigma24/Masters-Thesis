...
Tradycyjne algorytmy przetwarzania sygnałów w sieciach komórkowych---estymacja kanału, equalizacja, demapowanie, zewnępętla adaptacji łącza (OLLA)---są dobrze zrozumiałe i znormalizowane, lecz napotykają ograniczenia w złożonych scenariuszach propagacyjnych charakterystycznych dla 5G NR i planowanych systemów 6G (wysokie częstotliwości, massive MIMO, mobilność, slicing). Równolegle sztuczna inteligencja przenika warstwę radiową nie tylko jako narzędzie badawcze, ale jako przedmiot standaryzacji.

\subsection{Standaryzacja AI/ML w 3GPP}

3GPP w ramach Release~18 rozpoczął studium \emph{Study on Artificial Intelligence (AI)/Machine Learning (ML) for NR air interface} (TR~38.843)~\cite{3gpp-38843}. Raport obejmuje ogólny framework AI/ML oraz przypadki użycia: kompresję CSI w domenie przestrzenno-częstotliwościowej (modele dwustronne), predykcję CSI w domenie czasu (modele po stronie UE), zarządzanie wiązkami (beam management) i pozycjonowanie. W Release~19 prace kontynuowane są w kierunku wymagań normatywnych, metryk monitorowania (SGCS, NMSE, throughput) oraz mechanizmów fallbacku~\cite{3gpp-38843,ieee-comsoc-r18-ai}.

Kontekst standaryzacyjny uzasadnia badania nad kompresją CSI, predykcją kanału i odbiornikami neuronowymi nie jako modą technologiczną, lecz jako kierunkiem rozwoju interfejsu radiowego NR/6G.

\subsection{Walidacja systemowa w OpenAirInterface}

Wyniki uzyskane w symulatorze link-level (Sionna PHY) i system-level (Sionna SYS) wymagają walidacji poza środowiskiem czysto symulacyjnym. OpenAirInterface (OAI)~\cite{oai} stanowi open-source implementację stosu 5G RAN, w której możliwe jest wstrzyknięcie modeli ML po eksporcie do ONNX --- m.in. neural receiver w warstwie PHY~\cite{neural-rx-repo,sionna-rk} oraz autoenkoder CSI~\cite{cheng-oai-csi-2024}. Taka ścieżka pozwala zmierzyć latencję inferencji i jakość odbioru w konfiguracji zbliżonej do rzeczywistego gNB, bez konieczności badania harmonogramowania MAC.

\subsection{Motywacja badawcza}

Motywacją pracy jest dogłębne zrozumienie kompromisu między \emph{jakością odbioru}, \emph{zdolnością generalizacji} modeli ML oraz \emph{opóźnieniem inferencji} w kontekście wymagań 5G NR. Literatura pokazuje, że odbiorniki neuronowe mogą poprawiać BLER względem klasycznych łańcuchów~\cite{arxiv-2312-02601,arxiv-2409-02912}, lecz wdrożenie real-time wymaga świadomego projektowania architektury i optymalizacji sprzętowej (TensorRT, kwantyzacja)~\cite{arxiv-2409-02912,neural-rx-repo}. Jednocześnie jakość decyzji MCS zależy od dokładności i aktualności CSI~\cite{wen-csinet-2018,arxiv-2206-14383}, co uzasadnia badania cross-layer łączące moduły PHY i adaptację łącza w Sionna SYS~\cite{sionna-sys}.
...
\begin{enumerate}[label=\textbf{H\arabic*:}, leftmargin=*]
    ...
    \item \textbf{H3:} Integracja skompresowanej lub przewidywanej CSI poprawi skuteczność adaptacji łącza (throughput, śledzenie celu BLER) względem OLLA z idealnym CSI w co najmniej jednym scenariuszu z ograniczonym budżetem sprzężenia zwrotnego.
...
\bibentry{%
    ...
    \textbf{Wnioski dla pracy:} bezpośredni benchmark latencji i ścieżka integracji OAI; cel dla eksperymentów E5--E7.
...
\bibentry{%
    ...
    {Tutorial implementacji autoenkodera i transformera do CSI feedback w MATLAB, z metrykami NMSE i porównaniem do kodowania typu 2GPP.
...
Badania zorganizowane są warstwowo: każdy kolejny moduł rozszerza poprzedni, co zapewnia możliwość zatrzymania się na dowolnym etapie z kompletnym zestawem wyników.
...
Wzorzec~\cite{cheng-oai-csi-2024}: autoenkoder podzielony na enkoder (nrUE) i dekoder (gNB); inferencja ONNX Runtime C API na CPU ($\sim$18~MB). Mniejsze wymagania GPU; naturalny moduł cross-layer dla FDD massive MIMO (jeśli rozszerzony scope).
...
E7 & CSI compression & Autoencoder, ratio 1/4--1/32 & NMSE, SGCS, bits & 2GPP type II \\
...
\bottomrule
\caption{Plan eksperymentów badawczych.}
\end{longtable}
...
\subsection{Macierz generalizacji}
...
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Trening} & \textbf{Test} & \textbf{Moduł} & \textbf{Metryka główna} \\
\midrule
CDL-A, 0--20~dB, 3--30~km/h & CDL-A (seen) & A & BLER \\
CDL-A & CDL-C (unseen channel) & A, B & BLER / NMSE \\
CDL-A & $-5$, 25~dB (unseen SNR) & A & BLER \\
3--30~km/h & 100--300~km/h & A, C & BLER / NMSE pred. \\
Stały MCS & Zmienny MCS & A & BLER \\
\bottomrule
\end{tabular}
\caption{Scenariusze badania generalizacji.}
\end{table}
